...
from tensorflow.keras.layers import Conv1D, ReLU, MaxPool1D, UpSampling1D, Add, Input, Reshape
from tensorflow.keras.models import Model

# Kích thước đầu vào cố định
INPUT_SHAPE = (1, 4096) 
CHANNEL_AXIS = 1 # Kênh đầu tiên (Batch, Channels, Length)
...
def conv_block(input_tensor, filters, kernel_size=3, num_layers=5):
    ...
    for _ in range(num_layers):
        ...
        x = Conv1D(filters=filters, kernel_size=kernel_size, strides=1, padding='same', data_format='channels_first')(x)
        x = ReLU()(x)
    return x
...
def ModelNet1():
    ...
    # Reshape về (Batch, Length, Channels) cho Keras Conv1D chuẩn (Length first)
    # Tuy nhiên, ta dùng data_format='channels_first' để giữ định dạng (Batch, C, L)
    ...
    pool1 = MaxPool1D(pool_size=2, strides=2, data_format='channels_first')(conv1) # Output: (B, 64, 2048)
    
    # Block 2 (64 filters)
    conv2 = conv_block(pool1, filters=64, num_layers=5) # Output: (B, 64, 2048)

    # Pooling 2 (Max Pooling)
    pool2 = MaxPool1D(pool_size=2, strides=2, data_format='channels_first')(conv2) # Output: (B, 64, 1024)
    ...
    # Up-sampling 1
    up1 = UpSampling1D(size=2, data_format='channels_first')(latent) # Output: (B, 64, 2048)
    ...
    # Up-sampling 2
    up2 = UpSampling1D(size=2, data_format='channels_first')(conv4) # Output: (B, 64, 4096)
    ...
    outputs = Conv1D(filters=1, kernel_size=3, strides=1, padding='same', data_format='channels_first', activation='linear')(conv5) # Output: (B, 1, 4096)
