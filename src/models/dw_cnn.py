...
def dw_conv_block(input_tensor, filters, final_filters=None, num_layers=5):
    ...
    for _ in range(num_layers):
        ...
        x = Conv1D(filters=filters, kernel_size=3, strides=1, padding='same', data_format='channels_first')(x)
        x = ReLU()(x)
    
    # Lớp Conv cuối cùng (Block 3 và Block 4 có số filters khác biệt ở cuối)
    if final_filters is not None:
        x = Conv1D(filters=final_filters, kernel_size=3, strides=1, padding='same', data_format='channels_first')(x)
...
def DW_CNN():
    ...
    conv1 = dw_conv_block(inputs, filters=64, num_layers=5) # Output: (B, 64, 4096)
    
    # Lưu kết nối skip 1
    skip1 = conv1
    
    # Pooling 1 (Layer 6-7): DWT-based Pooling
    pool1 = DWT1D_Pooling()(conv1) # Output: (B, 128, 2048) - Bảng 2 liệt kê 128 filters
    
    # Block 2 (Layers 7-11): 5x Conv 1D, 64 filters
    conv2 = dw_conv_block(pool1, filters=64, num_layers=5) # Output: (B, 64, 2048)

    # Lưu kết nối skip 2
    skip2 = conv2

    # Pooling 2 (Layer 12-13): DWT-based Pooling
    pool2 = DWT1D_Pooling()(conv2) # Output: (B, 128, 1024) - Bảng 2 liệt kê 128 filters

    # Block 3 (Layers 13-18): Bottleneck. 5x Conv 1D (64 filters) + 1x Conv 1D (128 filters)
    # Tổng cộng 6 lớp Conv trong Block này.
    latent = dw_conv_block(pool2, filters=64, final_filters=128, num_layers=5) # Output: (B, 128, 1024)

    # --- DECODER (Up-sampling) ---
    
    # Up-sampling 1 (Layer 19-20): IDWT-based Up-sampling
    up1 = IDWT1D_UpSampling()(latent) # Output: (B, 64, 2048)

    # Add 1 (Layer 20-21): Skip connection từ Block 2 (skip2)
    add1 = Add()([up1, skip2]) 

    # Block 4 (Layers 21-26): 5x Conv 1D (64 filters) + 1x Conv 1D (128 filters)
    conv4 = dw_conv_block(add1, filters=64, final_filters=128, num_layers=5) # Output: (B, 128, 2048)

    # Up-sampling 2 (Layer 26-27): IDWT-based Up-sampling
    up2 = IDWT1D_UpSampling()(conv4) # Output: (B, 64, 4096)
    
    # Add 2 (Layer 27-28): Skip connection từ Block 1 (skip1)
    add2 = Add()([up2, skip1]) 

    # Output Block (Layers 28-31): 2x Conv 1D (64 filters) + 1x Conv 1D (1 filter)
    conv5 = Conv1D(filters=64, kernel_size=3, strides=1, padding='same', data_format='channels_first')(add2)
    conv5 = ReLU()(conv5)
    
    conv6 = Conv1D(filters=64, kernel_size=3, strides=1, padding='same', data_format='channels_first')(conv5)
    conv6 = ReLU()(conv6)

    # Final Output Layer (Layer 30-31/31): 1 filter, linear activation
    outputs = Conv1D(filters=1, kernel_size=3, strides=1, padding='same', data_format='channels_first', activation='linear')(conv6) # Output: (B, 1, 4096)
